...
class Matrix(object):
    ...
    def abs(self) -> Matrix:
        ...
        def _gradient():
            self.grad += List2D(self.nrow, self.ncol, abs(result.grad.data[0][0])) 
    ...
    def leakyrelu(self, negative_slope) -> Matrix:
        """Element-wise leaky rectilinear unit (leakyrelu)."""
        result = Matrix(self.data.leakyrelu(negative_slope), children(self,))

        def _gradient() -> None:
            self.grad += (result.data > 0) * result.grad + (result.data <= 0) * result.grad
