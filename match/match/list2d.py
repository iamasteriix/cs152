...
class List2D(object):
    ...
    def abs(self) -> float:
        return abs(self.data[i][j] for j in range(self.ncol) for i in range(self.nrow))
    ...
    def leakyrelu(self, negative_slope) -> List2D:
        """Element-wise leaky rectilinear unit function (leakyrelu)."""
        data = [
                [max(0.0, self.data[i][j]) + negative_slope * min(0.0, self.data[i][j]) for j in range(self.ncol)]
                for i in range(self.nrow)
                ]
        return List2D(*self.shape, data)
